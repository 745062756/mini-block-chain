...
int main() {
   ...
   while(1) {
      ...
      sendto(fd, &response, sizeof response, 0, (struct sockaddr*) &serverMAddress, serverMAddressSize);
      if (request.requestCode==FetchRecord || request.requestCode==PushRecord || request.requestCode==FetchList) printf("The ServerA finished sending the response to the Main Server.\n");
   }
}
...
void setupSocket() {
   ...
   if (bind(fd, (struct sockaddr *) &myAddress, sizeof myAddress)<0) {
      char warning[40];
      sprintf(warning, "Server A fails to bind port to %d", PORT_NUM);
      perror(warning);
      exit(1);
   }
}
...
void cacheFile() {
   ...
   char buf[1024];
   if ((filePTR = fopen("block1.txt", "r+"))==NULL) {
      perror("block1.txt");
      exit(1);
   }
